...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({
        submit: error.response?.data?.error || "Login failed. Check credentials.",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
        <div>
          ...
          <h2 className="mt-2 text-center text-3xl font-bold text-gray-900">Welcome Back</h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {/* Submit error */}
          {errors.submit && (
            <div className="rounded-md bg-red-50 p-4 text-red-700 text-sm">{errors.submit}</div>
          )}
